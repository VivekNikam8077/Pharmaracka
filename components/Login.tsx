...
const Login: React.FC<LoginProps> = ({ onLogin, users, settings, onSetIp }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Give some time for the animation before emitting
    setTimeout(() => {
      onLogin({ email, password });
      // Reset after a delay if auth fails (auth_failure listener in App.tsx will handle error)
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="block w-full px-6 py-4 bg-slate-100 dark:bg-slate-800 border-none rounded-2xl text-sm font-bold text-slate-800 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500"
              />
              {error && <p className="mt-2 text-red-500 text-[10px] font-black uppercase">{error}</p>}
            </div>

            <button
              type="submit"
              className="w-full py-5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl text-xs font-black uppercase tracking-[0.2em] shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all"
            >
             Login
            </button>
          </form>
