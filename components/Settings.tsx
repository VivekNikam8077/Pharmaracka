...
const Settings: React.FC<SettingsProps> = ({ settings, setSettings, users, socket }) => {
  ...
  // FIX: replace alert() with inline toast
  ...
  const handleSave = () => {
    ...
    // Optimistically update parent so UI reflects changes immediately
    setSettings(localSettings);
    // Show inline success toast for 2.5s
    setSaved(true);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-5 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold dark:text-white outline-none focus:ring-2 focus:ring-indigo-400"
              />
            </div>
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="https://..."
                  className="flex-grow px-5 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold dark:text-white outline-none focus:ring-2 focus:ring-indigo-400"
                />
                {localSettings.logoUrl && (
                  <img src={localSettings.logoUrl} className="w-12 h-12 rounded-lg object-contain bg-slate-100 p-1 flex-shrink-0" alt="Preview" />
                )}
              </div>
            </div>
            <div>
              ...
              {localSettings.loginBgUrl && (
                <div
                  className="mt-2 h-20 w-full rounded-xl bg-cover bg-center border border-slate-200 dark:border-slate-700"
                  style={{ backgroundImage: `url(${localSettings.loginBgUrl})` }}
                />
              )}
            </div>
          </div>
        ...
        <section className="bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 shadow-sm space-y-6">
          ...
          <div className="flex items-center justify-between p-6 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-700">
            ...
            <button
              ...
              className={`w-16 h-8 rounded-full transition-all relative flex-shrink-0 ${localSettings.darkMode ? 'bg-indigo-600' : 'bg-slate-300'}`}
            >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {localSettings.availableStatuses.map(status => (
              <div key={status} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700 rounded-xl group">
                <span className="text-xs font-bold text-slate-700 dark:text-slate-300 truncate mr-2">{status}</span>
                <button
      ...
      {/* Save button + inline toast */}
      <div className="flex flex-col items-center gap-4 pt-8">
        <button
          onClick={handleSave}
          className="flex items-center gap-3 px-12 py-5 bg-slate-900 dark:bg-slate-700 text-white rounded-[2rem] text-sm font-black uppercase tracking-[0.2em] shadow-2xl hover:bg-black dark:hover:bg-slate-600 hover:scale-105 active:scale-95 transition-all"
        >
          <Save className="w-5 h-5" /> Commit Global Configuration
        </button>

        {/* FIX: inline success feedback instead of alert() */}
        <div className={`flex items-center gap-2 text-emerald-600 font-black text-xs uppercase tracking-widest transition-all duration-300 ${saved ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2 pointer-events-none'}`}>
