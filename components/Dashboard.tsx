...
const INIT_FLAG_KEY = 'officely_init_flag_'; // ✅ NEW: Track initialization in localStorage
const TIME_UPDATE_INTERVAL = 5 * 60 * 1000; // 5 minutes
...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  // ✅ CRITICAL: Check if already initialized today (persists across navigation)
  ...
  // ✅ CRITICAL: Mark as initialized for today
  const markAsInitialized = (today: string) => {
    try {
      ...
      console.log('[Dashboard] 🔒 Marked session as initialized for', today);
    } catch (e) {
      console.error('[Dashboard] Failed to mark as initialized:', e);
    }
  };
  ...
  const saveSessionStats = (stats: SessionStats) => {
    ...
    try {
      ...
      localStorage.setItem(statsKey, JSON.stringify(toSave));
      console.log('[Dashboard] 💾 Saved stats:', toSave);
    } catch (e) {
  ...
  const sendPeriodicUpdate = () => {
    ...
    console.log('[Dashboard] ⏰ Periodic 5-minute update');
  ...
  // ✅ FIXED: Initialization with localStorage persistence
  useEffect(() => {
    ...
    // ✅ CRITICAL CHECK: Skip if already initialized today
    if (isAlreadyInitialized(today)) {
      console.log('[Dashboard] ⚠️ Already initialized for', today, '- skipping to prevent reset');
      
      // ✅ Still restore state from localStorage
      ...
      if (stored) {
        try {
          ...
          if (data.date === today && data.startTime) {
            // Restore state silently (no socket emission)
            ...
            console.log('[Dashboard] 📋 Restored state from localStorage (no socket emission)');
          }
        } catch (e) {
          console.error('[Dashboard] Failed to restore state:', e);
        }
      }
    ...
    console.log('[Dashboard] 🆕 First initialization for', today);
    ...
    if (stored) {
      try {
        ...
        if (data.date === today && data.startTime) {
          ...
          console.log('[Dashboard] 📋 Restored existing session:', {
            ...
            elapsedMinutes: Math.floor((now - data.startTime) / 60000)
          });
        } else {
          // New day - start fresh
          ...
          console.log('[Dashboard] 🆕 Started new session for new day');
        }
      } catch (e) {
        console.error('[Dashboard] Failed to restore session:', e);
      }
    } else {
      // No stored session - brand new
      const newStartTime = now;
      setSessionStartTime(newStartTime);
      setCurrentStatus(OfficeStatus.AVAILABLE);
      statusChangeTimeRef.current = newStartTime;
      ...
      console.log('[Dashboard] 🆕 Created brand new session');
    }
    
    // ✅ Emit initial status ONLY on first initialization
    socket.emit('status_change', {
      userId: user.id,
      userName: user.name,
      status: initialStatus,
    ...
    // ✅ CRITICAL: Mark as initialized (persists across navigation)
